...
public class PT_XMLReaderOG {
	...
	*/
	...
	public void Parse(string eS) {
		xmlText = eS;
		xml = new PT_XMLHashtableOG();
		Parse(eS, xml);
	}
	
	// This function will parse a possible series of tags
	void Parse(string eS, PT_XMLHashtableOG eH) {
		eS = eS.Trim();
		while(eS.Length > 0) {
			eS = ParseTag(eS, eH);
			eS = eS.Trim();
	...
	string ParseTag(string eS, PT_XMLHashtableOG eH) {
		// search for "<"
		int ndx = eS.IndexOf("<");
		...
		if (!eH.ContainsKey(tag)) {
			eH[tag] = new PT_XMLHashListOG();
		}
		// Create a hashtable to contain this tag's information
		PT_XMLHashListOG arrL = eH[tag] as PT_XMLHashListOG;
		//int thisHashIndex = arrL.Count;
		PT_XMLHashtableOG thisHash = new PT_XMLHashtableOG();
		arrL.Add(thisHash);
...
public class PT_XMLHashListOG {
	public ArrayList list = new ArrayList();
	
	public PT_XMLHashtableOG this[int s] {
		get {
			return(list[s] as PT_XMLHashtableOG);
		}
		set {
			list[s] = value;
		}
	...
	public void Add(PT_XMLHashtableOG eH) {
		list.Add(eH);
	}
...
public class PT_XMLHashtableOG {
	
	public List<string>				keys = new List<string>();
	public List<PT_XMLHashListOG>		nodesList = new List<PT_XMLHashListOG>();
	public List<string>				attKeys = new List<string>();
	public List<string>				attributesList = new List<string>();
	
	public PT_XMLHashListOG Get(string key) {
		int ndx = Index(key);
		if (ndx == -1) return(null);
		return( nodesList[ndx] );
	}
	
	public void Set(string key, PT_XMLHashListOG val) {
		int ndx = Index(key);
		if (ndx != -1) {
			nodesList[ndx] = val;
		} else {
	...
	public PT_XMLHashListOG this[string s] {
		get {
			return( Get(s) );
		}
		set {
