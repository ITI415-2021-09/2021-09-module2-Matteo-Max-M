...
public class DeckOG : MonoBehaviour {
...
[Header("Set in Inspector")]
	...
	// called by PyramidOG when it is ready
	...
	public void ReadDeck(string deckXMLText)
	{
		...
		PT_XMLHashListOG xDecos = xmlr.xml["xml"][0]["decorator"];
		Decorator deco;
		...
		PT_XMLHashListOG xCardDefs = xmlr.xml["xml"][0]["card"];
		...
		for (int i=0; i<xCardDefs.Count; i++) {
			...
			PT_XMLHashListOG xPips = xCardDefs[i]["pip"];
			if (xPips != null) {			
	...
	public Sprite GetFace(string faceS) {
	 ...
	 static public void Shuffle(ref List<Card> oCards)
	 {
	 	...
	 	{
	 		// find a random card, add it to shuffled list and remove from original deckOG
